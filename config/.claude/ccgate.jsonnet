{
  ...
  deny: [
    ...
    'Primary Checkout of Same Repo: When is_worktree=true, deny writes/checkouts under primary_checkout_root (shared .git). ccgate does NOT pass other sibling-worktree paths -- do NOT infer sibling worktrees from name similarity; paths outside both primary_checkout_root and repo_root are separate trusted repos (read/write + non-destructive git ok). Exception: removing stuck .git lock files (e.g. .git/index.lock, .git/worktrees/*/index.lock) under primary_checkout_root is allowed. deny_message: Accessing the primary checkout from a worktree is not allowed.',
  ],
