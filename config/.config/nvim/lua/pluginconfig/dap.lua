...
local actions = {
  ...
  toggle_bp = function() require('dap').toggle_breakpoint() end,
  set_bp_cond = function() require('dap').set_breakpoint(vim.fn.input('Breakpoint condition: ')) end,
  set_lp = function() require('dap').set_breakpoint(nil, nil, vim.fn.input('Log point message: ')) end,
  open_repl = function() require('dap').repl.open() end,
  run_last = function() require('dap').run_last() end,
  show_variables = function() require('telescope').extensions['dap'].variables() end,
  show_frames = function() require('telescope').extensions['dap'].frames() end,
  show_commands = function() require('telescope').extensions['dap'].commands() end,
}

M.keys = require('utils').lazy_keymap({
  {
    { 'n', '<Leader>dc',       actions.continue,       desc = 'Continue' },
    { 'n', '<Leader>dsv',      actions.step_over,      desc = 'Step over' },
    { 'n', '<Leader>dsi',      actions.step_into,      desc = 'Step into' },
    { 'n', '<Leader>dso',      actions.step_out,       desc = 'Step out' },
    { 'n', '<Leader>db',       actions.toggle_bp,      desc = 'Toggle breakpoint' },
    { 'n', '<Leader>dB',       actions.set_bp_cond,    desc = 'Add conditional breakpoint' },
    { 'n', '<Leader>dlp',      actions.set_lp,         desc = 'Add Logpoint' },
...
function M.status() return require('dap').status() end
